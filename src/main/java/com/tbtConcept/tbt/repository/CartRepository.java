...
//	@Query("SELECT c FROM Cart c WHERE c.user_id LIKE %:user_id%")
//	List<Cart> searchByCartLikeU(@Param("user_id") String user_id);
//	
//	@Query("SELECT c FROM Cart c WHERE c.product_id LIKE %:product_id%")
//	List<Cart> searchByCartLikeP(@Param("product_id") int product_id);
